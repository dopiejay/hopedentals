    ...
    <>
      ...
      <section className="bg-white px-6 py-20">
        ...
        <div className="mx-auto grid max-w-6xl grid-cols-1 gap-6 md:grid-cols-3">
          {branches.map((b) => (
            <div key={b.slug} className="group relative flex flex-col overflow-hidden rounded-3xl border border-stone/80 bg-white p-7 shadow-sm transition-all duration-300 hover:-translate-y-1 hover:shadow-xl hover:shadow-ink/10">
              <div className="absolute inset-x-0 top-0 h-1 bg-gradient-to-r from-shalom-navy to-shalom-teal" />
              <span className="mb-3 inline-block w-fit rounded-full bg-shalom-teal/10 px-3 py-1 text-[0.72rem] font-bold tracking-[0.1em] text-shalom-teal uppercase">
                {b.city}
              </span>
              <h3 className="mb-1.5 font-display text-xl font-bold text-ink">{b.name}</h3>
          ...
          {contactCards.map((c) => {
            ...
            return (
              ...
              >
                ...
                <h3 className="mb-1 font-display text-lg font-bold text-ink">{c.title}</h3>
